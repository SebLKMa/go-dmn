// Command volume-discounts-xml evaluates the tiered volume-discount pricing
// clause in contract-clause.md against a purchase volume, using the
// volume-discount.dmn.xml decision table and the dmn package. It is the
// same example as examples/volume-discounts, but the decision table is
// loaded from an OMG DMN XML file via dmn.DecodeXML instead of JSON.
package main
...
const dmnFileName = "volume-discount.dmn.xml"
...
func loadTable() (dmn.DecisionTableDefinition, error) {
	...
	f, err := os.Open(path)
	if err != nil {
		return dmn.DecisionTableDefinition{}, fmt.Errorf("opening %s: %w", path, err)
	}
	defer f.Close()
	def, err := dmn.DecodeXML(f)
	if err != nil {
		return dmn.DecisionTableDefinition{}, fmt.Errorf("decoding %s: %w", path, err)
	}
	return def, nil
}

func usage() {
	fmt.Fprintln(os.Stderr, "usage: volume-discounts-xml <purchase-volume>")
}
